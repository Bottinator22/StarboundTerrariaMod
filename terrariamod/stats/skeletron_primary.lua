...
function applyDamageRequest(damageRequest)
  ...
  local damage = 0
  local prot = status.stat("protection")
  local oneDamage = false
  if prot >= 100 then
      prot = 0
      oneDamage = true
  end
  if damageRequest.damageType == "Damage" or damageRequest.damageType == "Knockback" then
    damage = damage + root.evalFunction2("protection", damageRequest.damage, prot)
  ...
  if healthLost > 0 and damageRequest.damageType ~= "Knockback" then
    if oneDamage then
      status.modifyResource("health", -math.min(healthLost,1))
    else
  ...
  if oneDamage then
    return {{
        ...
        damageDealt = math.min(damage,1),
        healthLost = math.min(healthLost,1),
        hitType = hitType,
        kind = "Normal",

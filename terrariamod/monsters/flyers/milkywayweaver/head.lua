...
repeat
  ...
  if config.getParameter("flying", false) then -- some Milkyway Weavers fly, others don't
      self.inGround = true
  else
    ...
    world.sendEntityMessage(self.childId, "update", dt, (world.magnitude(mcontroller.velocity(), {0, 0}) - minSpeed) / (maxSpeed - minSpeed))
    end
...
function move()
  ...
  if speed > maxSpeed or speed < minSpeed then
   ...
   self.lastPosition = mcontroller.position()
      ...
      if ownerId() then
            if world.magnitude(world.entityPosition(ownerId()), mcontroller.position()) <= 10 then
                if not self.inGround then
                    ...
                    mcontroller.setVelocity(vec2.mul(mcontroller.velocity(), 0.95))
                end
                postMove()
                return
            end
    ...
    mcontroller.setVelocity(vec2.add(vec2.mul(mcontroller.velocity(), 0.999), toTarget))

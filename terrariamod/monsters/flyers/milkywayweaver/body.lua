...
self.headId = 0
self.childId = 0
self.segmentSize = config.getParameter("segmentSize")
self.segmentExtraSize = 0.25
...
setHealth(config.getParameter("ownerHealth"))
    ...
    message.setHandler("update", function(_, _, dt, speed)
        followOwner(speed)
        if world.entityExists(self.childId) then
            world.sendEntityMessage(self.childId, "update", dt, speed)
        end
  end)
  self.shouldDie = true
  self.notifications = {}
...
function update(dt)
  ...
  if not world.entityExists(self.childId) then
      ...
      spawnSegment(self.size, self.headId)
      end
  else
  ...
  status.addEphemeralEffect("weaverghost")
end
...
function followOwner(speed)
  if world.entityExists(self.ownerId) then
  local toBoss = (self.segmentSize + self.segmentExtraSize * speed)
  local ownerPos = world.entityPosition(self.ownerId)
  if isFirst then
      local ownerVel = world.entityVelocity(self.ownerId)
      ownerPos = vec2.add(ownerPos, vec2.mul(ownerVel, script.updateDt()))
  end
  local angle = vec2.angle(world.distance(mcontroller.position(), ownerPos))
...
function setVariables(ownerId, count, headId)
    ...
    self.headId = headId
    self.size = count
    if self.ownerId == self.headId then
        self.segmentSize = config.getParameter("segmentSize") + config.getParameter("firstSize", 0)
        isFirst = true
    end
    spawnSegment(count, headId)
    status.setStatusProperty("alpha", (count + 1) / 15 * 255)
end
...
function spawnSegment(count, tempHeadId)
    ...
    local segmentId = world.spawnMonster(config.getParameter("bodySegment"),mcontroller.position(), { level = monster.level(),ownerHealth = status.resourcePercentage("health"),ownerId = tempownerId, segmentsLeft = count - 1, headId = tempHeadId})
    self.childId = segmentId
    world.sendEntityMessage(segmentId, "damageTeam", entity.damageTeam())
  return true, segmentID
    else
    local tempownerId = entity.id()
    local segmentId = world.spawnMonster(config.getParameter("tailSegment"),mcontroller.position(), { level = monster.level(),ownerHealth = status.resourcePercentage("health"),ownerId = tempownerId, headId = tempHeadId})
    self.childId = segmentId
    world.sendEntityMessage(segmentId, "damageTeam", entity.damageTeam())
    return true, segmentID
    end

...
function followOwner(speed)
  if world.entityExists(self.ownerId) then
    local toBoss2 = world.magnitude(world.entityPosition(self.ownerId), mcontroller.position()) - (self.segmentSize + self.segmentExtraSize * speed)
...
function setVariables(ownerId, count, headId)
    ...
    self.headId = headId
    status.setStatusProperty("alpha", 1 / 15 * 255)
    --self.size = count

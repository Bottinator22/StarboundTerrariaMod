...
function die()
    ...
    local children = {}
    local params = {level=monster.level(),persistent = entity.persistent(), ownerUuid=config.getParameter("ownerUuid"), podUuid = config.getParameter("podUuid"), damageTeam=entity.damageTeam().team, damageTeamType = entity.damageTeam().type}
    for i=0,math.random() * 3 + 1 do
        table.insert(children, world.spawnMonster("starcellsmall", mcontroller.position(), params))
    end
    if capturable.ownerUuid() then
        capturable.disassociate()
        for k,v in next, children do
            local uuid = sb.makeUuid()
            world.setUniqueId(v, uuid)
            capturable.associate({
                name = world.entityName(v),
                description = world.entityDescription(v),
                portrait = world.entityPortrait(v, "full"),
                uniqueId = uuid,
                collisionPoly = world.callScriptedEntity(v, "mcontroller.collisionPoly"),
                status = world.callScriptedEntity(v, "capturable.captureStatus"),
                config = {
                    type = "starcellsmall",
                    parameters = params
                }
...
function move()
    if targetId then
        targetPos = world.entityPosition(targetId)
    else
        if ownerId() then
            local ownerPos = world.entityPosition(ownerId())
            if world.magnitude(ownerPos, mcontroller.position()) > 10 then
                targetPos = ownerPos
            else
                targetPos = mcontroller.position()
            end

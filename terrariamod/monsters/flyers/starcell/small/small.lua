...
local maxSpeed = 25
local progress = 0
local spawnMinion = false
-- Engine callback - called on initialization of entity
function init()
  self.shouldDie = true
  self.targets = {}
  targetPos = mcontroller.position()
  mcontroller.setVelocity({math.random() * 16 - 8,math.random() * 16 - 8})
  self.outOfSight = {}
  self.notifications = {}
...
function die()
    ...
    if spawnMinion then
        ...
        local params = {level=monster.level(),persistent = entity.persistent(), ownerUuid=config.getParameter("ownerUuid"), podUuid = config.getParameter("podUuid"), damageTeam=entity.damageTeam().team, damageTeamType = entity.damageTeam().type, uniqueId=uuid}
        local child = world.spawnMonster("starcell", mcontroller.position(), params)
        if capturable.ownerUuid() then
            capturable.dissociate()
            capturable.associate({
                name = world.entityName(child),
                description = world.entityDescription(child),
                portrait = world.entityPortrait(child, "full"),
                uniqueId = uuid,
                collisionPoly = world.callScriptedEntity(child, "mcontroller.collisionPoly"),
                status = world.callScriptedEntity(child, "capturable.captureStatus"),
                config = {
                    type = "starcell",
                    parameters = params
                }
...
function move()
    mcontroller.controlApproachVelocity({0, 0}, 25)
    mcontroller.setRotation(mcontroller.rotation() + 0.05 * progress)

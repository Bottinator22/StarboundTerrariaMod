...
function update(dt)
 ...
 repeat
    ...
    if not self.targetId or world.lineTileCollision(world.entityPosition(targetId), vec2.add(world.entityPosition(targetId), {0, -8}), {"Block"}) then
        table.remove(self.targets, 1)
        self.targetId = nil
    end
...
function debugGround(ground, targetPosition)
    world.debugLine(targetPosition, vec2.add(targetPosition, {0, -20}), "blue")
    if ground then
        world.debugLine(vec2.add(ground, {1, 0}), vec2.add(ground, {-1, 0}), "green")
    end
end
function move()
    function postMove()
  ...
  if speed > maxSpeed or speed < minSpeed then
    ...
    if self.lastPosition then
       if world.magnitude(self.lastPosition, mcontroller.position()) > maxSpeed * 2 then -- Either moved too fast (which shouldn't happen due to maxSpeed) or teleported
            mcontroller.setVelocity({0, 0})
       end
   end
   self.lastPosition = mcontroller.position()
    local toTarget = {0, 0}
    if false then -- not self.targetId
      if ownerId() then
    ...
    if ownerId() then
    if self.targetId and world.magnitude(world.entityPosition(ownerId()), mcontroller.position()) <= 60  then
        targetPosition = world.entityPosition(self.targetId)
    else
        self.targetId = nil
        local diff = world.distance(mcontroller.position(), world.entityPosition(ownerId()))
        local diffabs = {math.abs(diff[1]),math.abs(diff[2])}
        if diffabs[1] > 20 or diff[2] > 40 or diff[2] < -20 then
            targetPosition = world.entityPosition(ownerId())
            local ground = world.lineCollision(targetPosition, vec2.add(targetPosition, {0, -20}), {"Block"})
            debugGround(ground, targetPosition)
            if ground then
                targetPosition = vec2.add(ground, {0, 10})
            end
        else
            targetPosition = vec2.add(mcontroller.position(), {mcontroller.xVelocity() / 5, 0})
            local ground = world.lineCollision(targetPosition, vec2.add(targetPosition, {0, -20}), {"Block"})
            debugGround(ground, targetPosition)
            if ground then
                targetPosition = vec2.add(ground, {0, 10})

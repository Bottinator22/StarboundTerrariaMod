...
local segmentID = world.spawnMonster("eaterboss"..type,mcontroller.position(), { level = monster.level(), change = true, headId = config.getParameter("headId"), ownerId = self.ownerId, childId = self.childId, ownerHealth = status.resourcePercentage("health")})
    world.sendEntityMessage(segmentID, "damageTeam", entity.damageTeam())
    if world.entityExists(self.ownerId) then
        world.sendEntityMessage(self.ownerId, "childChange", segmentID)
    end
    if world.entityExists(self.childId) then
        world.sendEntityMessage(self.childId, "ownerChange", segmentID)
    end
    self.childId = nil
    self.ownerId = nil

...
self.children = {}
  ...
  self.anchorPoint = vec2.add(mcontroller.position(), {0, -2})
...
function move()
    ...
    self.parentPos = self.anchorPoint
    ...
    if self.targetId ~= nil then
        mcontroller.setRotation(vec2.angle(world.distance(world.entityPosition(self.targetId), self.anchorPoint)))
        ...
        if self.shootTime < 0 then
            self.shootTime = 150
            world.sendEntityMessage(world.spawnMonster("fungispore", mcontroller.position(), { level = monster.level(), ownerId = entity.id() }), "damageTeam", entity.damageTeam())
        end
    else

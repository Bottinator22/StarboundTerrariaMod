...
function spawnMonster(monster, offset)
    local ownerId = entity.id()
    world.spawnMonster(part, vec2.add(mcontroller.position(), offset), { level = monster.level(), ownerId = ownerId })
end
local deathtimer = 0
local life = 0
local alive = true
...
function init()
    enemies = config.getParameter("enemies", {})
self.offScreen = true
self.children = {}
  self.shouldDie = true
  self.targetId = nil
  ...
  -- graphical effects
  world.spawnMonster(config.getParameter("mainBackgroundPart", "moonlordbackground"), mcontroller.position(), {ownerId=entity.id()})
  world.spawnMonster("lunarpillarforeground", mcontroller.position(), {ownerId=entity.id(), type=config.getParameter("type", "stardust")})
end

function stopMusic()
end
function attack()
    -- overridden by pillar-exclusive scripts
end
function update(dt)

...
self.children = {}
  ...
  chargeTimer = config.getParameter("baseChargeTimer", 0) * 40
  ...
  monster.setAnimationParameter("chains", config.getParameter("chains"))
    ...
    mcontroller.setVelocity({(math.random() - 0.5) * 40, (math.random() - 0.5) * 40})
...
function move()
    if self.targetId == nil then
        ...
        if charging then
            self.targetPos = chargeTarget
            if world.magnitude(mcontroller.position(), chargeTarget) < 5 then
                charging = false
            end
        else
            if chargeTimer > 800 then
                charging = true
                chargeTarget = world.entityPosition(self.targetId)
            end
        end
        if chargeTimer > 800 then
            chargeTimer = 0
    ...
    if not charging then
        self.targetPos = world.entityPosition(ownerId)
    end
    ...
    mcontroller.setVelocity(vec2.add(vec2.mul(mcontroller.velocity(), 0.995), toTarget))

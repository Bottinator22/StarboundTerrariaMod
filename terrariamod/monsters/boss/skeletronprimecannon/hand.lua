...
self.children = {}
  ...
  self.shootingTarget = mcontroller.position()
  self.outOfSight = {}
...
function shoot()
    ...
    self.noShoot = true -- don't shoot twice in a row
...
function move()
  ...
  if self.phaseTime > 1 then
        if self.phase == "charge" then
            self.phase = "default"
            self.phaseTime = 0
        end
    end
    self.shootingTarget = vec2.add(mcontroller.position(), {0, self.offset})

...
function move()
    if not world.entityExists(self.children[1]) then
        status.setResourcePercentage("health", 0)
        return
    end
    if not world.entityExists(self.children[2]) then
        status.setResourcePercentage("health", 0)
        return
    end
    ...
    self.retinPos = world.entityPosition(self.children[2])
    local mag = world.magnitude(mcontroller.position(), self.retinPos)
    local dis = world.distance(self.retinPos, mcontroller.position())

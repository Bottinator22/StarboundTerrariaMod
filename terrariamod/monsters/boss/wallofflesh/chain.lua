...
-- chain.lua with start/end segments replaced with different frames
function update()
  ...
  if animateTimer > self.chains[1].animTime then
        animateTimer = 0
        if frame == #self.chains[1].segmentImages then
            frame = 1
        else
            frame = frame + 1
        end
    ...
    if not continue and (not chain.targetEntityId or world.entityExists(chain.targetEntityId)) then
      ...
      if segmentCount > 0 then
        ...
        for i = 1, lastDrawnSegment do
          local image = chain.segmentImages[frame]

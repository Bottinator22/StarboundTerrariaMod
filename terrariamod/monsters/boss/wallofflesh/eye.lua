...
self.baseTurnSpeed = 0.1
  ...
  offset = config.getParameter("offset")
  message.setHandler("health", function(_,_,health)
        lastHealth = health
        status.setResourcePercentage("health", health)
  end)
  message.setHandler("state", function(_,_,state)
        top = state.top
        bottom = state.bottom
  end)
  ...
  status.setStatusProperty("headId", ownerId)
end
function sendHealth(health)
    world.sendEntityMessage(ownerId, "health", health, entity.id())
end
function update(dt)
...
function defaultMove()
    if self.targetId then
        ...
        local offsetCompare = top
        if offset < 0 then
            offsetCompare = bottom
        end
        local diff = offsetCompare[2] - self.targetPos[2]
        diff = diff * math.abs(offset)
        self.targetPos[2] = self.targetPos[2] + diff
        ...
        if self.summonTime <= 0 then
            self.summonTime = 90 * status.resourcePercentage("health") + 10
            ...
            spawnLeveled.spawnProjectile("bluelaser", mcontroller.position(), entity.id(), toTarget, false, {level=monster.level(), power=8 - 4 * status.resourcePercentage("health")})
...
function move()
    ...
    mcontroller.setXPosition(ownerPos[1])
    mcontroller.setVelocity({0, math.min(math.max(self.targetPos[2] - mcontroller.yPosition(), approachSpeed * -1), approachSpeed)})

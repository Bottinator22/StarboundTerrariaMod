...
self.baseTurnSpeed = 0.1
  ...
  message.setHandler("state", function(_,_,state)
        flipped = state.moveDir < 0
        if state.vertMovement > 0 then
            movingDir = movingSpeed
        elseif state.vertMovement < 0 then
            movingDir = movingSpeed * -1
        end
  end)
...
function move()
    ...
    local diff = world.distance(ownerPos, mcontroller.position())
    if math.abs(diff[2]) > 15 then
        movingDir = math.min(math.max(diff[2], movingSpeed * -1), movingSpeed)
    end
    mcontroller.setVelocity({0, movingDir})
    if math.abs(diff[2]) > 50 then
        mcontroller.setPosition(ownerPos)
    end
    animator.setFlipped(flipped)
    animator.resetTransformationGroup("beam")
    animator.scaleTransformationGroup("beam", 200)
    local height = 200
    if flipped then

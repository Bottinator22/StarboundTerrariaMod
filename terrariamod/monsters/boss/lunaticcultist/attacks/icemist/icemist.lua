...
function move()
    ...
    if life % 30 == 20 then
        ...
        for i=0,math.pi * 2,math.pi * 2 / 6 do
            local dir = i
            local approach = {math.cos(dir), math.sin(dir)}
            spawnLeveled.spawnProjectile("cultisticeshard", mcontroller.position(), entity.id(), approach, false, {level=monster.level(), power=7})
        end
    end
    mcontroller.setRotation(mcontroller.rotation() + spinSpeed)
    mcontroller.setVelocity(direction)

...
self.children = {}
  ...
  animator.scaleTransformationGroup("trailscale", {0, 0})
...
function move()
    status.addEphemeralEffect("scripttransparent")
    if life > lifetime then
        status.setResource("health", 0)
    else
        life = life + 1
    end
    status.setStatusProperty("alpha", 192 - life / lifetime * 192)
    animator.resetTransformationGroup("trailscale")
    animator.scaleTransformationGroup("trailscale", {0.25, 0.25})
    animator.scaleTransformationGroup("trailscale", {1 + life / lifetime / 4, 1 + life / lifetime / 4})
end

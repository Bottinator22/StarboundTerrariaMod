...
self.children = {}
  self.shouldDie = true
  self.notifications = {}
  direction = config.getParameter("direction", {0, 0})
...
function move()
    if not target then
        local limit = vec2.add(vec2.mul(direction, 160), mcontroller.position())
        target = world.lineCollision(mcontroller.position(), limit) or limit
    end
    life = life + 1
    if life > 30 then
        status.setResource("health", 0)
    end
    local dis = world.magnitude(mcontroller.position(), target)
    if dis <= maxLength then
        length = dis
        local dir = vec2.angle(world.distance(target, mcontroller.position()))
        mcontroller.setRotation(dir)
    else
        length = maxLength
        local dir = vec2.angle(direction)
        mcontroller.setRotation(dir)
    end
    ...
    animator.scaleTransformationGroup("beam", {length * 2, 1})
    animator.rotateTransformationGroup("beam", mcontroller.rotation())
    if life >= 3 then
        animator.setAnimationState("body", "idle")
        setDamageSources()
    end
    if life == 3 and dis > maxLength then
        local dir = vec2.angle(world.distance(target, mcontroller.position()))

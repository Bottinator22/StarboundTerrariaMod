...
function move()
    if self.ownerId ~= nil then
        ...
        if world.pointCollision({math.floor(mcontroller.position()[1]+0.5), math.floor(mcontroller.position()[2]+0.5)}, {"Block", "Platform", "Dynamic", "Slippery"}) then
            mcontroller.setPosition(world.entityPosition(self.ownerId))
        else
            self.free = true
        end
        end

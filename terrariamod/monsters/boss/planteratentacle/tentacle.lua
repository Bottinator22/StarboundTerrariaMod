...
self.children = {}
  ...
  self.parent = config.getParameter("ownerId", ownerId())
  self.targets = {}
  ...
  self.maxSpeed = 35
  self.targetDir = math.random() * math.pi * 2
...
function move()
    ...
    self.targetPosOffsetDelay = self.targetPosOffsetDelay - 1
    if self.targetPosOffsetDelay <= 0 then
        self.targetDir = self.targetDir + (math.random() - 0.5) * math.pi
        self.targetPosOffsetDelay = math.random() * 400
    end
    ...
    mcontroller.setRotation(self.targetDir)
    
    local approach = {math.cos(dir), math.sin(dir)}
    local toParent = vec2.mul(approach,mag / 10)
    mcontroller.setVelocity(vec2.add(mcontroller.velocity(), toParent))
    
    local base = {math.cos(self.targetDir), math.sin(self.targetDir)}
    local offset = vec2.mul(base, 10)
    self.targetPos = vec2.add(offset, self.parentPos)

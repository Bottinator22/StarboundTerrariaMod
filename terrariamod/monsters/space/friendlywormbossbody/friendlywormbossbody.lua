...
function update(dt)
  ...
  if not world.entityExists(self.childId) then
      ...
      if world.entityType(self.childId) ~= "friendlywormbossbody" and world.entityType(self.childId) ~= "friendlywormbosstail" then
      --spawnSegment(self.size)
      end
  end
  else
...
function spawnSegment(count)
  ...
  local stepAngle = (math.pi * 2) / count
    ...
    local segmentId = world.spawnMonster("friendlywormbossbody", vec2.add(mcontroller.position(), offset), { level = monster.level(),ownerHealth = status.resourcePercentage("health")})
  ...
  local stepAngle = (math.pi * 2) / count
    local offset = vec2.rotate({1,0}, stepAngle * 1)
    local segmentId = world.spawnMonster("friendlywormbosstail", vec2.add(mcontroller.position(), offset), { level = monster.level(),ownerHealth = status.resourcePercentage("health")})
    table.insert(segments, segmentId)
...
function releaseProbe()
    self.probe = false
    self.behavior = nil
    self.probeId = world.spawnMonster("friendlyprobe", mcontroller.position(), { level = monster.level()})
end
